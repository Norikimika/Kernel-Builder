...
| KernelSU | An object that contains information about whether the KernelSU patch is used, including repository details, branch, and other related settings. |
| KernelSUSFS | An object that contains information about whether the KernelSUSFS feature is enabled, including the branch used. |
| enableLXC | A Boolean value indicating whether to enable `Docker` support. |
| Extra | An object that contains additional kernel build options, such as dtboMIUI, and undervolting (uv). |
...
"AnyKernel3": {
   "release": true,
   "custom": {
     "repo": "https://github.com/Norikimika/AnyKernel3/",
     "branch": "munch",
     "strname": "Error by Norikimika"
   }
}
```

In this configuration, I used the customized `AnyKernel3` for packaging.

`release` A boolean value indicating whether the packaged flash package will be released. This option only takes effect when set to `true`.
`custom` An object containing custom configuration options for AnyKernel3.
`repo` The URL of the AnyKernel3 repository.
`branch` The branch of the AnyKernel3 repository to use.
`strname` A custom name or string assigned to the kernel package.

### KernelSU

The KernelSU object controls whether the KernelSU patch is applied. It includes additional configuration options such as:
...
`"lkm"` Determines whether KernelSU should be loaded as a Loadable Kernel Module (LKM).
`"patch"` Indicates whether the kernel should be patched for KernelSU functionality.

### KernelSUSFS

The `"KernelSUSFS"` object controls the support for KernelSU's special filesystem. This includes configuration options such as:
...
`"enable"` Enables or disables KernelSUSFS. When set to `true`, KernelSUSFS is enabled, and when set to `false`, it is disabled.
`"branch"` Specifies the branch used for the KernelSUSFS implementation. This allows you to choose a specific branch for the KernelSUSFS code.
...
The `"Extra"` object contains additional kernel build options:

`"dtboMIUI"` Applies MIUI-specific `dtsi` modifications for compatibility.
`"uv"` Enables or disables undervolting support to reduce power consumption.
